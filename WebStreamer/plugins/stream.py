...
@StreamBot.on(NewMessage(func=lambda e: e.message.file and e.is_private, forwards=True))
async def media_receive_handler(event: NewMessage.Event):
    user = await event.get_sender()
    
    # Log that a message was received. This is a crucial diagnostic step.
    logging.info(f"Received a file message from user ID: {user.id}. Attempting to process...")
    ...
    try:
        ...
        logging.error("An unexpected error occurred in media_receive_handler: %s", e, exc_info=True)
